...
window.onload = function() {
    ...
    objectStore.openCursor().onsuccess = function(event) {
      var cursor = event.target.result;
        ...
        if(cursor) {
          ...
          listItem.innerHTML = cursor.value.taskTitle + ' — ' + cursor.value.hours + ':' +
              cursor.value.minutes + ', ' + cursor.value.month + ' ' + cursor.value.day + daySuffix + ' ' +
              cursor.value.year + '.';
  ...
  function addData(e) {
    ...
    // required attributes
    if(title.value == '' || hours.value == null || minutes.value == null || day.value == '' ||
        month.value == '' || year.value == null) {
      ...
      var newItem = [
        { taskTitle: title.value, hours: hours.value, minutes: minutes.value, day: day.value,
            month: month.value, year: year.value, notified: "no" }
      ];
      ...
      var objectStoreRequest = objectStore.add(newItem[0]);
        objectStoreRequest.onsuccess = function(event) {
          ...
          // clear the form, ready for adding the next entry
            var newDate = new Date().getMonth().toString();
          title.value = '';
          hours.value = null;
          minutes.value = null;
          day.value = 01;
    ...
    objectStore.openCursor().onsuccess = function(event) {
      var cursor = event.target.result;
        ...
        switch(cursor.value.month) {
          ...
          // check if the current hours, minutes, day, month and year values match the stored values
            // for each task in the IDB.
          // The + operator in this case converts numbers with leading zeros into their non leading
            // zero equivalents, so e.g.
          // 09 -> 9. This is needed because JS date number values never have leading zeros, but our
            // data might.
          // The secondsCheck = 0 check is so that you don't get duplicate notifications for the same
            // task. The notification
          // will only appear when the seconds is 0, meaning that you won't get more than one
            // notification for each task
          if(+(cursor.value.hours) == hourCheck && +(cursor.value.minutes) == minuteCheck &&
              +(cursor.value.day) == dayCheck && monthNumber == monthCheck && cursor.value.year ==
              yearCheck && cursor.value.notified == "no") {
